...
var removeBlobs = function(data, callback) {
  function _removeBlobs(obj, curKey, containingObject) {
    ...
    } else if (obj && 'object' == typeof obj && !isBuffer(obj)) { // and object
      for (var key in obj) {
...
Socket.prototype.emit = function (ev) {
  ...
  if (hasBinary_1(args)) { parserType = socket_ioParser.BINARY_EVENT; } // binary
...
class Auth {
  ...
  loginWithToken({ token, appId, email }, cb) {
    ...
    this.io.sails.initialConnectionHeaders = headers;
    console.log('$$$' this.io.socket, this.io.sails)
    this.restClient.headers = Object.assign({}, this.restClient.headers, headers);
    cb({ status: 'ok' }, null);
  }
